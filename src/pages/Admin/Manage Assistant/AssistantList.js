...
const AssistantList = ({ assistants, loading, error, onEdit, onDelete }) => {
  ...
  if (error) return <Typography color="error">Failed to load assistants: {error}</Typography>;
        ...
        <Card key={assistant._id} sx={{ marginBottom: '10px', boxShadow: 1 }}>
          ...
          >
            <Box>
              ...
              <Typography variant="body2">Username: {assistant.username}</Typography>
            ...
            <Box>
              ...
              <IconButton aria-label="delete" onClick={() => onDelete(assistant._id)}>
                <DeleteIcon />
              </IconButton>
            </Box>
          </CardContent>
